...
test_harness_1.test('sending an email', async (server, assert) => {
    ...
    assert.equal(e.body['Destination.ToAddresses.member.1'], 'my-friend@gmail.com');
    assert.equal(e.body['Message.Body.Text.Data'], 'my body');
    assert.equal(e.body['Message.Subject.Data'], 'my subject');
});
test_harness_1.test('sending multiple emails', async (server, assert) => {
    // Send in background
    const p = (async () => {
...
test_harness_1.test('sending raw email', async (server, assert) => {
    ...
    assert.equal(e.body['Source'], 'me@gmail.com');
    assert.equal(e.body['Destinations.member.1'], 'To:my-friend@gmail.com');
